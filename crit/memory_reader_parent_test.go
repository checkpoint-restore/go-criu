...
func TestMemoryReaderParentChainAndCycle(t *testing.T) {
	...
	writeTestInventory(t, parentDirectory, crtoolsImagesV1_2, CompressionBlock, 0)
	...
	legacyPresent := &pagemap.PagemapEntry{
		...
		Blocks:        testPagemapBlocks([]uint32{uint32(len(encodedParentPage))}, 1),
	}
...
func TestMemoryReaderMixedParentCompressionLayers(t *testing.T) {
	...
	grandparentEntry.Blocks = testPagemapBlocks([]uint32{uint32(len(encodedGrandparent))}, 1)
	...
	parentEntry.Blocks = testPagemapBlocks([]uint32{uint32(len(encodedParent))}, 1)
	writeTestPagemap(
		t,
		parentDirectory,
		pid,
