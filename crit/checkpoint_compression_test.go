...
func TestCompressAndDecompressCheckpoint(t *testing.T) {
	...
	if !compressed.Changed || compressed.Compression.Mode != CompressionPerPage {
	...
	if err != nil || info.Mode != CompressionPerPage || info.ImageVersion != crtoolsImagesV1_2 {
		t.Fatalf("compression info = %+v, %v", info, err)
	}
	entries := compressionTestReadPagemap(t, directory)
	if got := entries[0].GetCompressedSize(); len(got) != 3 || got[0] != 0 || got[1] >= compressionTestPageSize || got[2] != compressionTestPageSize {
...
func TestCheckpointCompressionIdempotence(t *testing.T) {
	for _, operation := range []string{"compress", "decompress"} {
		t.Run(operation, func(t *testing.T) {
			...
			if operation == "compress" && result.Compression.Mode != CompressionPerPage {
...
func TestCompressionAlignsFirstRawPayloadOfLaterEntry(t *testing.T) {
	...
	entries := compressionTestReadPagemap(t, directory)
	if len(entries[0].GetCompressedSize()) != 1 {
...
func TestDecompressCheckpointRegionBlocks(t *testing.T) {
	directory := t.TempDir()
	...
	entry.CompressedSize = []uint32{uint32(len(encoded)), 0}
	entry.TotalCompressedSize = proto.Uint64(uint64(len(encoded)))
	entry.RegionPages = proto.Uint32(2)
	compressionTestWriteCheckpoint(t, directory, CompressionRegion, []*pagemap.PagemapEntry{entry}, encoded)
	...
	if !bytes.Equal(got, want) {
		t.Fatal("region decompression produced incorrect pages")
	}
}

func TestDecompressCheckpointRegionFinalShortCompressedBlockWithoutTotal(t *testing.T) {
	directory := t.TempDir()
	firstRegion := append(bytes.Repeat([]byte{'A'}, compressionTestPageSize), bytes.Repeat([]byte{'B'}, compressionTestPageSize)...)
	finalPage := bytes.Repeat([]byte{'C'}, compressionTestPageSize)
	encodedFinalPage := compressTestBlock(t, finalPage)
	entry := compressionTestPagemapEntry(0x1000, 3)
	entry.CompressedSize = []uint32{uint32(len(firstRegion)), uint32(len(encodedFinalPage))}
	entry.RegionPages = proto.Uint32(2)
	stored := append(append([]byte(nil), firstRegion...), encodedFinalPage...)
	compressionTestWriteCheckpoint(t, directory, CompressionRegion, []*pagemap.PagemapEntry{entry}, stored)
	...
	want := append(append([]byte(nil), firstRegion...), finalPage...)
	got, err := os.ReadFile(filepath.Join(directory, "pages-1.img"))
	if err != nil {
		t.Fatal(err)
	}
	if !bytes.Equal(got, want) {
		t.Fatal("decompression mishandled the short final region")
	}
}
...
func TestDecompressCheckpointRawAndZeroRuns(t *testing.T) {
	...
	entry.TotalCompressedSize = proto.Uint64(uint64(len(stored)))
	compressionTestWriteCheckpoint(t, directory, CompressionPerPage, []*pagemap.PagemapEntry{entry}, stored)
...
func TestCheckpointCompressionRejectsInvalidInventoryRegionMetadata(t *testing.T) {
	tests := []struct {
		name  string
		entry *inventory.InventoryEntry
	}{
		{
			name: "outside-region-mode",
			entry: &inventory.InventoryEntry{
				ImgVersion: proto.Uint32(crtoolsImagesV1_1), CompressRegionSize: proto.Uint32(compressionTestPageSize),
			},
		},
		{
			name: "missing",
			entry: &inventory.InventoryEntry{
				ImgVersion: proto.Uint32(crtoolsImagesV1_2), Compress: proto.Uint32(uint32(CompressionRegion)),
			},
		},
		{
			name: "zero",
			entry: &inventory.InventoryEntry{
				ImgVersion: proto.Uint32(crtoolsImagesV1_2), Compress: proto.Uint32(uint32(CompressionRegion)), CompressRegionSize: proto.Uint32(0),
			},
		},
		{
			name: "unaligned",
			entry: &inventory.InventoryEntry{
				ImgVersion: proto.Uint32(crtoolsImagesV1_2), Compress: proto.Uint32(uint32(CompressionRegion)), CompressRegionSize: proto.Uint32(compressionTestPageSize + 1),
			},
		},
		{
			name: "too-large",
			entry: &inventory.InventoryEntry{
				ImgVersion: proto.Uint32(crtoolsImagesV1_2), Compress: proto.Uint32(uint32(CompressionRegion)), CompressRegionSize: proto.Uint32(uint32(maxCompressedRegionSize) + compressionTestPageSize),
			},
		},
	}
	for _, test := range tests {
...
func TestDecompressCheckpointKeepsV12WithParentReference(t *testing.T) {
	...
	entry.TotalCompressedSize = proto.Uint64(0)
	compressionTestWriteCheckpoint(t, directory, CompressionPerPage, []*pagemap.PagemapEntry{entry}, nil)
...
func compressionTestWriteCheckpointNamed(t *testing.T, directory, pagemapName string, pagesID uint32, mode CompressionMode, entries []*pagemap.PagemapEntry, pages []byte) {
	...
	if mode.Compressed() {
		...
		if mode == CompressionRegion {
			invEntry.CompressRegionSize = proto.Uint32(2 * compressionTestPageSize)
		}
	}
	compressionTestWriteImage(t, filepath.Join(directory, "inventory.img"), &CriuImage{
