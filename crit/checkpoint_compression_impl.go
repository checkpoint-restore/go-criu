...
// compressCheckpoint converts raw pages to CRIU's page-sized block format.
func compressCheckpoint(ctx context.Context, checkpointDir string, opts CompressOptions) (result CheckpointCompressionResult, retErr error) {
	...
	inv.entry.Compress = proto.Uint32(uint32(CompressionBlock))
	inv.entry.CompressBlockSize = proto.Uint32(uint32(pageSize))
	...
	if commitErr == nil || errors.Is(commitErr, ErrCheckpointCompressionCleanup) {
		...
		result.Compression = CompressionInfo{
			ImageVersion:   crtoolsImagesV1_2,
			Mode:           CompressionBlock,
			BlockSizeBytes: uint32(pageSize),
		}
	}
	return result, commitErr
}

// decompressCheckpoint expands page-sized or multi-page compressed blocks.
func decompressCheckpoint(ctx context.Context, checkpointDir string, opts DecompressOptions) (result CheckpointCompressionResult, retErr error) {
	...
	inv.entry.CompressBlockSize = nil
...
func compressionLoadInventory(directory string, metadata map[string]*compressionFileMetadata) (*compressionInventory, error) {
	...
	if mode != CompressionOff && mode != CompressionBlock {
	...
	if mode == CompressionBlock {
		if blockSize != 0 &&
			...
			return nil, fmt.Errorf("inventory has invalid compression block size %d", blockSize)
		}
	} else if entry.CompressBlockSize != nil {
		return nil, errors.New("inventory records a compression block size while compression is off")
	}
	info := CompressionInfo{ImageVersion: version, Mode: mode, BlockSizeBytes: blockSize}
...
func compressionStageCompressedPages(ctx context.Context, pm *compressionPagemap, metadata *compressionFileMetadata, pageSize uint64, rawRanges []compressionRange) (compressionStagedFile, PagemapCompressionStats, error) {
	...
	stage, err := compressionStageFromSource(pm.pagesPath, metadata, func(source, output *os.File) error {
		...
		for _, entry := range pm.entries {
			...
			blockSizes := make([]uint32, 0, int(min(nrPages, uint64(1024*1024))))
			...
			for pageIndex := uint64(0); pageIndex < nrPages; pageIndex++ {
				...
				blockSizes = append(blockSizes, uint32(storedSize))
				totalPayload += uint64(storedSize)
				outputOffset += uint64(storedSize)
				stats.OutputBytes += uint64(storedSize)
			}
			allRaw := len(blockSizes) != 0
			...
			if allRaw {
				...
				entry.Blocks = &pagemap.PagemapBlocks{
					BlockSizes:       blockSizes,
					TotalPayloadSize: proto.Uint64(totalPayload),
					PagesPerBlock:    proto.Uint32(1),
				}
			}
		}
		extra := make([]byte, 1)
...
func compressionStageDecompressedPages(ctx context.Context, pm *compressionPagemap, metadata *compressionFileMetadata, pageSize uint64) (compressionStagedFile, PagemapCompressionStats, error) {
	...
	stage, err := compressionStageFromSource(pm.pagesPath, metadata, func(source, output *os.File) error {
		...
		for _, entry := range pm.entries {
			...
			blockSizes := blocks.GetBlockSizes()
			pagesPerBlock := uint64(blocks.GetPagesPerBlock())
			remainingPages := nrPages
			for blockIndex := 0; blockIndex < len(blockSizes); {
				if err := ctx.Err(); err != nil {
					return err
				}
				encodedSize := uint64(blockSizes[blockIndex])
				blockPages := min(pagesPerBlock, remainingPages)
