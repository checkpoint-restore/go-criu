...
func FuzzIndexMemoryLayer(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, data []byte) {
		...
		if data[10]&0x80 != 0 {
			entry.RegionPages = uint32Pointer(uint32(data[10] & 0x0f))
		}
		...
		for len(encoded) >= 2 {
			...
			entry.CompressedSize = append(entry.CompressedSize, size)
		...
		if data[11]&0x01 != 0 {
			...
			entry.TotalCompressedSize = uint64Pointer(total)
		}
		...
		expectedOffsets := (len(entry.CompressedSize) + int(memoryBlockOffsetStride) - 1) / int(memoryBlockOffsetStride)
		...
		for blockIndex := range entry.CompressedSize {
