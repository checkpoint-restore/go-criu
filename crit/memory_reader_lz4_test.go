...
func TestMemoryReaderAcrossSparseBlockStride(t *testing.T) {
	...
	for index := 0; index < blocks; index++ {
		...
		case 0:
			...
			blockSizes[index] = uint32(len(encoded))
		...
		case 2:
			...
			blockSizes[index] = pageSize
			payload = append(payload, page...)
		}
		expected = append(expected, page...)
	}
	entry.Blocks = testPagemapBlocks(blockSizes, 1)
...
func TestMemoryReaderPageBlockZeroRawAlignedAndSearch(t *testing.T) {
	const (
		pageSize = 4096
		pid      = 17
		pagesID  = 3
	...
	first.Blocks = testPagemapBlocks([]uint32{0, uint32(len(encoded))}, 1)
	...
	if !bytes.Equal(memory.Bytes(), expected) {
		t.Fatal("decoded page-block memory does not match")
	}
...
func TestMemoryReaderMultiPageAndCorruptBlock(t *testing.T) {
	const (
		pageSize = 4096
		pid      = 23
		base     = 0x20000
	...
	entry.Blocks = testPagemapBlocks([]uint32{uint32(len(encoded)), pageSize}, 2)
	...
	// Both pages are stored in the same compressed block. Reading them in one
	// session exercises the decoded-block cache for the second page.
	cachedBlock, err := reader.GetMemPages(base+19, base+2*pageSize-7)
	if err != nil {
		t.Fatal(err)
	}
	wantCachedBlock := append(append([]byte(nil), pageA[19:]...), pageB[:pageSize-7]...)
	if !bytes.Equal(cachedBlock.Bytes(), wantCachedBlock) {
		t.Fatal("decoded pages from a cached block do not match")
	}
	...
	if !bytes.Equal(memory.Bytes(), expected) {
		t.Fatal("decoded block memory does not match")
	}

	corrupt := t.TempDir()
	writeTestInventory(t, corrupt, crtoolsImagesV1_2, CompressionBlock, 0)
	bad := presentTestEntry(base, 1)
	bad.Blocks = testPagemapBlocks([]uint32{1}, 1)
...
func TestMemoryReaderRejectsCompressedBlockDecodedSizeMismatch(t *testing.T) {
	...
	// two-page block. The decoder must reject a successful short decode.
	encoded := compressTestBlock(t, bytes.Repeat([]byte{'M'}, pageSize))
	entry := presentTestEntry(base, 2)
	entry.Blocks = testPagemapBlocks([]uint32{uint32(len(encoded))}, 2)
	writeTestPagemap(t, directory, pid, pagesID, entry)
...
func TestMemoryReaderBlockZeroRawAndFinalShortBlock(t *testing.T) {
	const (
		pageSize = 4096
		pid      = 29
		pagesID  = 7
	...
	writeTestPagemap(t, directory, pid, pagesID, entry)
	payload := append(append([]byte(nil), rawBlock...), encodedFinalPage...)
	if err := os.WriteFile(
	...
	expected := append(make([]byte, 2*pageSize), rawBlock...)
	expected = append(expected, finalPage...)
	if !bytes.Equal(memory.Bytes(), expected) {
		t.Fatal("decoded zero, raw, and final short blocks do not match")
	}
...
func TestMemoryReaderCompressedPayloadAlignment(t *testing.T) {
	...
	first.Blocks = testPagemapBlocks([]uint32{uint32(len(encodedFirst))}, 1)
	second := presentTestEntry(base+pageSize, 1)
	second.Flags = uint32Pointer(pePresent | pePayloadAligned)
	second.Blocks = testPagemapBlocks([]uint32{uint32(len(encodedSecond))}, 1)
...
func TestMemoryReaderRejectsTruncatedOrOversizedPayload(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			entry.Blocks = testPagemapBlocks([]uint32{uint32(len(encoded))}, 1)
